...
# Note: can't differentiate through this, rewrite in the style above if need Zygote gradients:
...
TreeADAM(x::Dict{String, Dict{String, Dict{String, Dict{String, Float32}}}},
    ...
    TreeADAM(η, β, ϵ, [β[1], β[2]], zerocopy(x), zerocopy(x))
...
function adam_step!(o::TreeADAM, x, Δ) 
    ...
    println("beta_power ", o.beta_power, " eta ", η)
    (min_grad, max_grad) = min_max_dict(Δ)
    println("min grad ", min_grad, " max grad ", max_grad)
    (min_mt, max_mt) = min_max_dict(o.mt)
    println("min mt ", min_mt, " max mt ", max_mt)
    (min_vt, max_vt) = min_max_dict(o.vt)
    println("min vt ", min_vt, " max vt ", max_vt)
    (min_correction, max_correction) = min_max_dict(corrected_term)
    println("min correction ", min_correction, " max correction ", max_correction)
    (min_x, max_x) = min_max_dict(x)
    println("min weight ", min_x, " max weight ", max_x)
